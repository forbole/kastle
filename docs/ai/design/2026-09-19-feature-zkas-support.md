...
flowchart LR
  ...
  Core -->|FVK and account token| Daemon[User-selected walletd]
  Daemon -->|prepared bundle| Core
  Core -->|verified signatures| Daemon
```

Background code owns ZKas derivation, watch registration, daemon calls, and signing. UI and page code receive only public address, balance/sync state, and transaction IDs. Kaspa and ZKas network settings remain distinct.
...
- Walletd: `POST /api/wallet/watch`, `GET /api/status`, `GET /api/wallet/balance`, `POST /api/wallet/prepare`, `POST /api/wallet/submit`. Never use custodial `create/import/reveal/send`.
...
- Background service: keyring and account guard, signer, browser handlers.
...
- Long remote proofs show progress and permit only one in-flight payment.
