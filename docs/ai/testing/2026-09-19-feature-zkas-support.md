...
M10 token-requiring proxy compatibility: the new status-probe test failed with HTTP 401 when the temporary token was removed, then passed with a fresh 32-character lowercase hex token. All 72 focused ZKas tests pass; TypeScript compile, Chrome and Firefox builds, signer integrity check, and ESLint pass. ESLint reports 38 existing warnings and zero errors. Astra found no actionable Kastle issue. Codex Security diff scan `9629e6b6-5fd3-433c-a7f1-b4007e2657f6` reviewed both changed source files with zero findings; its snapshot preceded this testing-document update. `https://zkwd.mooncake.space/api/status` still returns HTTP 520, so a live public-daemon connection and funded payment are not verified.
