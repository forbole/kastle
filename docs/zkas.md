...
The wallet switcher selects a recovery-phrase, imported-key, or Ledger wallet and one of its accounts; selecting a wallet does not change the active network. When Experimental features is enabled, the address dropdown beside the selected account shows its ZKas Mainnet address alongside its Kaspa and EVM addresses. Recovery-phrase accounts without passphrases derive a ZKas address from their selected account index; imported-key account 0 shows its ZKas address only after a ZKas spending seed is attached. Ledger wallets, passphrase wallets, and imported Kaspa keys without an attached ZKas seed have no ZKas address row. The address dropdown and receive screen derive this address locally without querying the daemon. Open the ZKAS asset screen to read its synced balance.
