...
Walletd's _recoverable history_ setting is optional and controlled on the daemon. Kastle reads its status but does not enable it automatically. With the setting off, historical outgoing recipients and memos may not be available; enabling it later does not automatically restore earlier details without a complete rescan. The daemon can observe shielded activity and retained history, so choose and configure one you trust. See the [walletd history documentation](https://github.com/firecash/zkas-rusty/blob/main/docs/WALLETD.md) for its privacy and rescan options.
