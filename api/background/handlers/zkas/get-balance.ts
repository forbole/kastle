...
export const zkasGetBalanceHandler: Handler = async (_tabId, message, sendResponse) => {
  ...
  const assertConnected = async () => {
    ...
    if (!hasZKasConnection(connections, origin, selection)) throw new Error("Connect this website to ZKas first");
  };
  await assertConnected();
  if (!credentials.daemonUrl) throw new Error("Configure a ZKas daemon in Kastle first");
  const guard = async () => {
    await assertConnected();
    await zkasKeyService.checkSelection(selection, credentials.daemonUrl, credentials.keyringVersion);
  ...
  await guard();
  const client = new ZKasClient({ baseUrl: credentials.daemonUrl, token: credentials.walletToken, network: credentials.network, guard });
  const state = await client.state(credentials.fullViewingKeyHex, credentials.address);
  await guard();
