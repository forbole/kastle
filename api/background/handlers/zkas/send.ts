...
export const zkasSendHandler: Handler = async (
  ...
  if (account.network !== "mainnet")
    throw new Error("ZKas website payments require mainnet");
  if (!input.to.startsWith("zkas:"))
  ...
  try {
    ...
    if (popup.id === undefined)
      throw new Error("Unable to open ZKas payment approval");
    await zkasDappPendingStore.bindWindow(approvalId, popup.id);
    await browser.alarms.create(`${ZKAS_DAPP_ALARM_PREFIX}${approvalId}`, {
      when: Date.now() + ZKAS_DAPP_TIMEOUT_MS,
    });
