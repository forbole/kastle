...
export default function ZKasReceive() {
  ...
  if (!account) {
    return <div className="p-4"><Header title="Receive ZKAS" /><p role="status">{error || (loading ? "Loading address…" : "Enable Experimental features and select a supported wallet account.")}</p></div>;
  }
  return <ReceiveAddress address={account.address} chainName="ZKas Mainnet" iconUrl={zkasIcon} />;
}
