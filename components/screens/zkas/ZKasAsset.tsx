...
export default function ZKasAsset() {
  ...
  const [paymentRecord, setPaymentRecord] =
    useState<ZKasPaymentRecord | null>();
    ...
    const load = async () => {
      try {
        ...
        if (account.network !== "mainnet") {
          setMessage(
            "Testnet receive only: the pinned signer cannot authorize testnet payments.",
          );
        }
        try {
          ...
          if (
            record &&
            (record.selection.walletId !== account.walletId ||
              record.selection.accountIndex !== account.accountIndex ||
              record.selection.network !== account.network)
          ) {
            throw new Error(
              "Selected ZKas account changed. Refresh this screen.",
            );
          }
          ...
          if (active)
            setPaymentError(
              cause instanceof Error
                ? cause.message
                : "Unable to load payment recovery state",
            );
        }
        try {
          ...
          if (state.missingHistory)
            setMessage("Wallet history is incomplete. Balance is not final.");
          else if (!state.synced)
            setMessage("ZKas daemon is still syncing. Balance is not final.");
  ...
  const clearPayment = async () => {
    if (
      !paymentRecord ||
      !window.confirm(
        "I checked ZKas history and understand this payment may have been broadcast. Clear the warning and allow another send?",
      )
    )
      return;
    try {
      ...
      setPaymentError(
        cause instanceof Error
          ? cause.message
          : "Unable to clear payment warning",
      );
    }
  };
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4 text-white">
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        <button
          type="button"
          className="rounded-lg border border-daintree-700 px-3 py-2 text-sm font-semibold"
          onClick={() => setIsMenuOpen(true)}
        >
          {kaspaAccount?.name ?? "Account"}
        </button>
      ...
      <p className="mb-3 text-center text-xs text-icy-blue-400">
        ZKas Mainnet · Experimental
      </p>
      <div className="space-y-4">
        <div className="rounded-xl border border-daintree-700 bg-daintree-800 p-5">
          ...
          <button
            ...
            onClick={() =>
              void setSettings((prev) => ({
                ...prev,
                hideBalances: !prev.hideBalances,
              }))
            }
          >
          ...
          <p className="mt-3 text-sm text-daintree-400" role="status">
            {message}
          </p>
        </div>
        {address && (
          <p className="break-all rounded-xl bg-daintree-800 p-3 text-xs text-daintree-200">
            {address}
          </p>
        )}
        {!address && (
          <p className="text-sm text-daintree-200">
            Choose a ZKas wallet from the wallet switcher, or import a spending
            seed in Import Wallet.
          </p>
        )}
        <div className="flex gap-2">
          <button
            className="flex-1 rounded-full bg-icy-blue-400 p-3 font-semibold disabled:opacity-40"
            disabled={!address}
            onClick={() => navigate("/receive/zkas")}
          >
            Receive
          </button>
          <button
            ...
            disabled={
              ...
              (paymentRecord !== null && paymentRecord.status !== "success")
            }
        ...
        {paymentRecord && paymentRecord.status !== "success" && (
          <div
            role="alert"
            className="space-y-2 rounded-xl border border-amber-500 p-3 text-sm"
          >
            <p>
              A previous ZKas payment is{" "}
              {paymentRecord.status === "preparing"
                ? "still preparing or interrupted"
                : paymentRecord.status === "submitting"
                  ? "possibly being submitted"
                  : "unresolved"}
              . Check recent activity before another send.
            </p>
            {paymentRecord.txid && (
              <p className="break-all text-xs">
                Possible transaction ID: {paymentRecord.txid}
              </p>
            )}
            {paymentError && <p className="text-red-400">{paymentError}</p>}
            <button
              type="button"
              className="rounded-full border border-amber-500 px-3 py-2"
              onClick={() => void clearPayment()}
            >
              I checked history; clear warning
            </button>
          </div>
        )}
        {paymentRecord?.status === "success" && paymentRecord.txid && (
          <p className="break-all text-xs text-daintree-400">
            Last submitted ZKas payment: {paymentRecord.txid}
          </p>
        )}
        <button
        ...
        >
          Configure ZKas daemon
        </button>
        {(wallet?.type === "privateKey" || wallet?.type === "zkasSeed") &&
          address && (
            <button
              className="w-full rounded-full border border-daintree-700 p-3"
              onClick={() => {
        ...
        <section aria-label="Recent ZKas activity" className="space-y-2">
          ...
          {history &&
            ...
            !history.pendingOutgoing?.length && (
              <p className="text-xs text-daintree-400">
                No recent activity reported by the daemon.
              </p>
            )}
        </section>
        <p className="text-xs text-daintree-400">
          Your chosen daemon can see your full viewing key and shielded
          activity. Kastle keeps the spending seed local.
        </p>
      </div>
    </div>
  );
