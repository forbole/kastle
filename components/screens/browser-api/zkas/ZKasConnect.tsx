...
export default function ZKasConnect() {
  ...
  const approve = async () => {
    ...
    setError("");
    try {
      if (!isAllowedZKasDappOrigin(origin) || !requestId)
        throw new Error("Invalid ZKas connection request");
      const current = await getZKasPublicAccount();
      if (
        current.address !== account.address ||
        current.walletId !== account.walletId ||
        current.accountIndex !== account.accountIndex ||
      ...
      await ApiExtensionUtils.sendMessage(
        requestId,
        ApiUtils.createApiResponse(requestId, current),
      );
      window.close();
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "Unable to connect ZKas website",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4 text-white">
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm">
          Allow this website to view the selected ZKas address and request your
          shielded balance. Every payment still needs a separate approval.
        </p>
        <p className="break-all rounded-lg bg-daintree-800 p-3 text-xs">
          {account?.address ?? "Loading ZKas account…"}
        </p>
        {error && (
