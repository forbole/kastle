...
export default function LegacyFeatures() {
  const navigate = useNavigate();
  const [settings, setSettings] = useSettings();

  return (
    <div className="flex h-full flex-col gap-4 p-6">
      <Header title="Legacy features" onClose={() => navigate("/dashboard")} />
      <div
      ...
      >
        <div className="flex items-center justify-between">
          <span className="font-semibold text-daintree-200">
            Legacy features are intended for recovering funds from older Kastle
            wallet versions. Only activate if you know what you are doing.
          </span>
        </div>
      </div>
