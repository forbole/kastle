...
import { KRC721TransferFormData } from "@/components/screens/KRC721Transfer.tsx"; // Types for props

interface KNSTransferFailureProps {
  transactionIds?: string[] | undefined;
}

export default function KRC721TransferFailure({
  transactionIds,
}: KNSTransferFailureProps) {
  const navigate = useNavigate();
  const { watch } = useFormContext<KRC721TransferFormData>();
  const { networkId } = useRpcClientStateful();
  const { tick, tokenId } = watch();
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex flex-col gap-2 text-center">
            ...
            <span className="px-2 text-sm text-gray-500">
              {`The carriage couldn't deliver your ${tick} #${tokenId} this time. Please check the recipient’s address or try again later`}
            </span>
          </div>
