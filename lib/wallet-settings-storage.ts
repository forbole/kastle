...
export async function withWalletSettingsLock<T>(operation: () => Promise<T>): Promise<T> {
  if (!navigator.locks) throw new Error("Wallet update locking is unavailable in this browser");
...
export async function updateWalletSettingsLocked<T>(
  ...
  if (key !== WALLET_SETTINGS_STORAGE_KEY) throw new Error("Invalid wallet settings key");
  return withWalletSettingsLock(async () => {
    const stored = await storage.getItem<T>(key);
    const current = stored ?? options.fallback;
    if (current === undefined) throw new Error("Wallet settings are unavailable");
    if (options.expectedJson !== undefined && JSON.stringify(current) !== options.expectedJson) {
