...
function assertExpectedAccount(credentials: ZKasCredentials, expectedAccount?: PublicZKasAccount): void {
  if (expectedAccount && (credentials.address !== expectedAccount.address ||
...
export async function clearZKasPaymentRecord(record: ZKasPaymentRecord): Promise<void> {
  await internal(Method.ZKAS_PAYMENT_CLEAR, { selection: record.selection, id: record.id });
}

export async function getZKasState(expectedAccount?: PublicZKasAccount): Promise<ZKasState> {
...
export async function getZKasHistory(expectedAccount?: PublicZKasAccount): Promise<ZKasHistory> {
  const credentials = await internal<ZKasCredentials>(Method.ZKAS_GET_CREDENTIALS);
  assertExpectedAccount(credentials, expectedAccount);
  const client = await permittedClient(credentials);
  ...
  try {
    ...
    const signer: ZKasSigner = {
      ...
      verifyAndSign: async (prepared) => {
        ...
        const result = await internal<{ signatures: { index: number; sig: string }[] }>(
          Method.ZKAS_SIGN,
          request,
        );
    ...
    try {
      ...
      if (submitting) {
        ...
        throw cause instanceof ZKasSubmissionUncertainError ? cause : new ZKasSubmissionUncertainError(txid);
      }
      await internal(Method.ZKAS_PAYMENT_RELEASE, { selection, id: record.id }).catch(() => undefined);
      throw cause;
    }
  } finally {
