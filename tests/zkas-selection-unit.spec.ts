...
import {
  ...
  assertUniqueZKasSeed,
  attachZKasSeed,
  getSelectedAvailableZKasAddress,
  getSelectedZKasAccount,
  getZKasMnemonic,
  getZKasSecretSource,
  listZKasSwitchAccounts,
  loadSelectedZKasAccount,
  requireSelectedZKasAddress,
  sameZKasSelection,
} from "@/lib/zkas/selection";
...
test("selected phrase account follows the Kastle wallet and selected ZKas mainnet", () => {
  ...
  expect(sameZKasSelection(mainnet, { ...mainnet, network: "testnet" })).toBe(
    false,
  );
});
...
test("ZKas account requests require the experimental toggle and active ZKas network", () => {
  for (const settings of [
    ...
    expect(() =>
      getSelectedZKasAccount(walletSettings, settings, true),
    ).toThrow(/select ZKas|experimental/i);
  }
  expect(() =>
    getSelectedZKasAccount(walletSettings, mainnetSettings, false),
  ).toThrow(/experimental/i);
});

test("unsupported or absent account secrets fail before derivation", () => {
  const selected = getSelectedZKasAccount(
    walletSettings,
    mainnetSettings,
    true,
  );
  expect(
    getZKasMnemonic(
      [{ id: "wallet-1", type: "mnemonic", value: "synthetic phrase" }],
      selected,
    ),
  ...
  expect(() =>
    getSelectedZKasAccount(
      { ...walletSettings, selectedAccountIndex: 2 },
      mainnetSettings,
      true,
    ),
  ).toThrow(/account/i);
...
test("an explicitly attached ZKas seed enables only the selected imported-key account", () => {
    ...
    wallets: [
      {
        ...
        accounts: [
          { index: 0, name: "Imported key", address: "kaspa:example" },
        ],
      },
    ],
  ...
  expect(getZKasSecretSource(secrets, selected)).toEqual({
    type: "seed",
    value: seed,
  });
  expect(secrets[0].value).toBe("02".repeat(32));
  expect(() => attachZKasSeed(secrets, selected, "03".repeat(32))).toThrow(
    /already/i,
  );
  expect(() =>
    getZKasSecretSource(secrets, { ...selected, accountIndex: 1 }),
  ).toThrow(/account/i);
});
...
test("a Kaspa private key is never silently treated as a ZKas seed", () => {
  ...
  ).toThrow(/not supported|import/i);
  expect(() =>
    attachZKasSeed(
      [{ id: "wallet-1", type: "privateKey", value: "02".repeat(32) }],
      selected,
...
test("an interrupted import recovers its encrypted seed without making a second wallet ID", () => {
  ...
  const recovered = addOrRecoverZKasSeed(
    [orphan],
    walletSettings,
    seed,
    "retry-id",
  ...
  expect(addOrRecoverZKasSeed([], walletSettings, seed, "new-id")).toEqual({
    id: "new-id",
    secrets: [{ id: "new-id", type: "zkasSeed", value: seed }],
  });
  ...
  expect(() =>
    addOrRecoverZKasSeed([orphan], persistedWallet, seed, "retry-id"),
  ).toThrow(/already imported/i);
});
...
test("address selector derives only the selected recovery-phrase account", async () => {
  ...
  const result = await getSelectedAvailableZKasAddress(
    wallets,
...
test("address selector shows an attached ZKas seed only for its selected wallet", async () => {
  ...
  const wallets = {
    ...
    wallets: [
      ...
      },
      {
        ...
        backed: true,
        accounts: [
          { index: 0, name: "Passphrase", address: "kaspa:passphrase" },
        ],
      },
      {
        id: "ledger",
  ...
  for (const selectedWalletId of ["kaspa-only", "passphrase", "ledger"]) {
    expect(
      await getSelectedAvailableZKasAddress(
...
test("a standalone ZKas seed is an independent wallet and leaves the selected phrase unchanged", async () => {
  ...
  const derive = async (source: {
    type: "mnemonic" | "seed";
    value: string;
  }) => (source.type === "seed" ? "zkas:shielded" : "zkas:phrase");

  expect(
    (await getSelectedAvailableZKasAddress(wallets, secrets, derive))?.address,
  ).toBe("zkas:phrase");
  ...
  expect(getZKasSecretSource(secrets, shieldedSelection)).toEqual({
    type: "seed",
    value: seed,
  });
  expect(
    await getSelectedAvailableZKasAddress(
      {
        ...wallets,
        selectedWalletId: "shielded-wallet",
        selectedAccountIndex: 0,
      },
      secrets,
      derive,
...
test("a delayed address response cannot show another wallet or account", async () => {
  ...
  const pending = new Promise<typeof expected & { address: string }>(
    (resolve) => {
      finishRequest = resolve;
    },
  );
  const display = pending.then((account) =>
    requireSelectedZKasAddress(account, expected),
  );
  finishRequest({
    ...expected,
    walletId: "wallet-2",
    address: "zkas:other-wallet",
  });
  await expect(display).rejects.toThrow(/selected ZKas account changed/i);

  expect(() =>
    requireSelectedZKasAddress(
      { ...expected, accountIndex: 2, address: "zkas:other-account" },
      expected,
    ),
  ).toThrow(/selected ZKas account changed/i);
  expect(() =>
    requireSelectedZKasAddress(
      { ...expected, network: "testnet", address: "zkas:other-network" },
      expected,
    ),
  ).toThrow(/selected ZKas account changed/i);
  expect(
    requireSelectedZKasAddress(
      { ...expected, address: "zkas:selected" },
      expected,
    )?.address,
  ).toBe("zkas:selected");
...
test("daemon permission pattern matches the selected host and refuses plaintext remote URLs", () => {
  expect(getZKasDaemonOriginPattern("https://daemon.example/path")).toBe(
    "https://daemon.example/*",
  );
  expect(getZKasDaemonOriginPattern("http://127.0.0.1:8080")).toBe(
    "http://127.0.0.1/*",
  );
  expect(() => getZKasDaemonOriginPattern("http://daemon.example")).toThrow(
    /https/i,
  );
  expect(() =>
    getZKasDaemonOriginPattern("https://user:pass@daemon.example"),
...
test("only messages from the extension origin reach privileged ZKas handlers", () => {
  ...
  expect(
    isTrustedZKasSender({ id, url: `${origin}/popup.html` }, id, origin),
  ).toBe(true);
  for (const sender of [
...
test("lock or unlock during asynchronous selection reads invalidates the request", async () => {
  for (const unlockedAgain of [false, true]) {
    ...
    const pendingSettings = new Promise<WalletSettings>((resolve) => {
      finishRead = resolve;
    });
    let unlocked = true;
    let version = 1;
    ...
    const selected = loadSelectedZKasAccount(
      keyring,
      () => pendingSettings,
      async () => mainnetSettings,
      async () => true,
    );
    unlocked = false;
    version += 1;
