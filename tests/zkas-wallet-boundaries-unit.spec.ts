...
import type { WalletInfo, WalletSettings } from "@/contexts/WalletManagerContext";
import { getSelectedKaspaAccount, listKaspaWallets } from "@/lib/wallet-switcher-selection";
import { updateWalletSettingsLocked, WALLET_SETTINGS_STORAGE_KEY, withWalletSettingsLock } from "@/lib/wallet-settings-storage";
...
test("Kaspa wallet switcher excludes standalone ZKas seeds", () => {
  const wallets = (["mnemonic", "privateKey", "ledger", "zkasSeed"] as const).map((type) => ({
    ...
    accounts: [{ index: 0, name: "Account 0", address: type === "zkasSeed" ? "zkas:shielded" : "kaspa:transparent" }],
  })) satisfies WalletInfo[];
  expect(listKaspaWallets(wallets).map((wallet) => wallet.type))
    .toEqual(["mnemonic", "privateKey", "ledger"]);
});

test("Kaspa account lookup never returns a selected ZKas address", () => {
  const phrase = { id: "phrase", type: "mnemonic" as const, name: "Recovery phrase", backed: true,
    accounts: [{ index: 0, name: "Account 0", address: "kaspa:phrase" }] };
  const shielded = { id: "shielded", type: "zkasSeed" as const, name: "ZKas seed", backed: true,
    accounts: [{ index: 0, name: "Account 0", address: "zkas:shielded" }] };
  const settings: WalletSettings = {
    wallets: [phrase, shielded], selectedWalletId: "shielded", selectedAccountIndex: 0,
    lastRecoveryPhraseNumber: 1, lastPrivateKeyNumber: 0,
  };
  expect(getSelectedKaspaAccount(settings)).toBeNull();
  expect(getSelectedKaspaAccount({ ...settings, selectedWalletId: "phrase" })?.address)
    .toBe("kaspa:phrase");
});
...
test("two windows starting from one wallet snapshot cannot overwrite each other", async () => {
  ...
  const initial: WalletSettings = {
    wallets: [], selectedWalletId: undefined, selectedAccountIndex: undefined,
    lastRecoveryPhraseNumber: 0, lastPrivateKeyNumber: 0,
  };
  ...
  Object.assign(globalThis, { storage: {
    getItem: async () => persisted,
    setItem: async (_key: string, value: WalletSettings) => { persisted = value; },
  } });
  try {
    const baseline = JSON.stringify(initial);
    const add = (id: string) => updateWalletSettingsLocked<WalletSettings>(
      WALLET_SETTINGS_STORAGE_KEY,
      (settings) => ({ ...settings, wallets: [...settings.wallets, {
        id, type: "zkasSeed", name: id, backed: true,
        accounts: [{ index: 0, name: "Account 0", address: `zkas:${id}` }],
      }] }),
    ...
    await Promise.all(["second", "third"].map((id) => updateWalletSettingsLocked<WalletSettings>(
      WALLET_SETTINGS_STORAGE_KEY,
      (current) => ({ ...current, wallets: [...current.wallets, {
        id, type: "zkasSeed", name: id, backed: true,
        accounts: [{ index: 0, name: "Account 0", address: `zkas:${id}` }],
      }] }),
    )));
    expect(persisted.wallets.map((wallet) => wallet.id)).toEqual(["first", "second", "third"]);
  } finally {
...
test("reset waits for a seed import to finish updating both stores", async () => {
  ...
  const staged = new Promise<void>((resolve) => { importStaged = resolve; });
  let continueImport!: () => void;
  const holdImport = new Promise<void>((resolve) => { continueImport = resolve; });
  Object.defineProperty(navigator, "locks", { configurable: true, value: {
    request: (_name: string, operation: () => Promise<unknown>) => {
      const result = tail.then(operation);
      tail = result.catch(() => undefined);
  ...
  try {
    ...
    const importWallet = updateWalletSettingsLocked<WalletSettings>(
      WALLET_SETTINGS_STORAGE_KEY,
      async (current) => {
        const result = addOrRecoverZKasSeed(secrets, current, seed, "seed-wallet");
        secrets = result.secrets;
        importStaged();
        await holdImport;
        return { ...current, selectedWalletId: result.id, selectedAccountIndex: 0,
          wallets: [...current.wallets, { id: result.id, type: "zkasSeed", name: "ZKas seed", backed: true,
            accounts: [{ index: 0, name: "Account 0", address: "zkas:shielded" }] }] };
      },
    );
    await staged;
...
test("failed wallet metadata write preserves the encrypted seed for import retry", async () => {
  ...
  try {
    ...
    const importWallet = () => updateWalletSettingsLocked<WalletSettings>(
      WALLET_SETTINGS_STORAGE_KEY,
      (current) => {
        const result = addOrRecoverZKasSeed(secrets, current, seed, crypto.randomUUID());
        secrets = result.secrets;
        return { ...current, wallets: [...current.wallets, { id: result.id, type: "zkasSeed",
          name: "ZKas seed", backed: true,
          accounts: [{ index: 0, name: "Account 0", address: "zkas:shielded" }] }] };
      },
    );
...
test("removing a previously sole wallet preserves a newly imported wallet", async () => {
  ...
  const phrase = { id: "phrase", type: "mnemonic" as const, name: "Recovery phrase", backed: true,
    accounts: [{ index: 0, name: "Account 0", address: "kaspa:phrase" }] };
  const seedWallet = { id: "seed", type: "zkasSeed" as const, name: "ZKas seed", backed: true,
    accounts: [{ index: 0, name: "Account 0", address: "zkas:shielded" }] };
  const initial: WalletSettings = { wallets: [phrase], selectedWalletId: "phrase", selectedAccountIndex: 0,
    lastRecoveryPhraseNumber: 1, lastPrivateKeyNumber: 0 };
  let persisted = initial;
  let secrets: WalletSecret[] = [{ id: "phrase", type: "mnemonic", value: "phrase" }];
  let tail: Promise<unknown> = Promise.resolve();
  let importStaged!: () => void;
  const staged = new Promise<void>((resolve) => { importStaged = resolve; });
  let continueImport!: () => void;
  ...
  try {
    const importing = updateWalletSettingsLocked<WalletSettings>(WALLET_SETTINGS_STORAGE_KEY, async (current) => {
    ...
    const removing = removeWalletAndSecretLocked(
      "phrase", { ...initial, wallets: [], selectedWalletId: undefined, selectedAccountIndex: undefined },
      async (id) => { secrets = secrets.filter((secret) => secret.id !== id); },
      async () => { secrets = []; },
    );
    continueImport();
...
test("failed removal metadata write never deletes the wallet secret", async () => {
  const originalLocks = navigator.locks;
  const originalStorage = (globalThis as { storage?: unknown }).storage;
  const wallet = { id: "seed", type: "zkasSeed" as const, name: "ZKas seed", backed: true,
    accounts: [{ index: 0, name: "Account 0", address: "zkas:shielded" }] };
  const initial: WalletSettings = { wallets: [wallet], selectedWalletId: "seed", selectedAccountIndex: 0,
    lastRecoveryPhraseNumber: 0, lastPrivateKeyNumber: 0 };
  ...
  Object.assign(globalThis, { storage: {
    getItem: async () => persisted,
    setItem: async () => { throw new Error("metadata unavailable"); },
  } });
  try {
    await expect(removeWalletAndSecretLocked(
      "seed", { ...initial, wallets: [], selectedWalletId: undefined, selectedAccountIndex: undefined },
      async () => { secretRemoved = true; },
      async () => { secretRemoved = true; },
...
test("ambiguous secret deletion does not restore a wallet with a missing key", async () => {
  ...
  try {
    ...
    await expect(removeWalletAndSecretLocked(
      "seed", { ...initial, wallets: [], selectedWalletId: undefined, selectedAccountIndex: undefined },
      async () => { throw new Error("unexpected individual removal"); },
      ambiguousReset,
