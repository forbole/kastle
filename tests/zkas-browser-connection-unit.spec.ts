...
test("ZKas website grant is exact to origin, wallet, account, and network", () => {
  ...
  expect(
    hasZKasConnection(connections, "http://localhost:4174", selected),
  ).toBe(false);
  expect(
    hasZKasConnection(connections, origin, {
      ...selected,
      walletId: "wallet-b",
    }),
  ).toBe(false);
  expect(
    hasZKasConnection(connections, origin, { ...selected, accountIndex: 0 }),
  ).toBe(false);
  expect(
    hasZKasConnection(connections, origin, { ...selected, network: "testnet" }),
  ).toBe(false);
...
test("content script accepts only messages sent by its own page", () => {
  ...
  expect(isTrustedZKasPageMessage({} as Window, origin, pageWindow)).toBe(
    false,
  );
  expect(
    isTrustedZKasPageMessage(
      pageWindow,
      "https://attacker.example",
      pageWindow,
    ),
...
test("overlapping website grant mutations preserve revocations and order", async () => {
  ...
  await Promise.all([
    store.remove(origin),
    store.remove("https://second.example"),
  ]);
...
test("two origins reusing one page request ID cannot share an approval channel", () => {
  ...
  const second = createZKasApprovalSession(
    "attacker-chosen-id",
