...
test("ZKas is absent until Experimental features is enabled", () => {
  ...
  expect(() =>
    selectWalletNetwork(initialSettings, ZKAS_MAINNET, null),
  ).toThrow(/experimental/i);
  expect(
    getVisibleWalletNetworks({ ...initialSettings, preview: true }, null),
  ).toContain(ZKAS_MAINNET);
});

test("network switching keeps Kaspa transport and ZKas selection distinct", () => {
  const testnet = selectWalletNetwork(
    initialSettings,
    NetworkType.TestnetT10,
    null,
  );
  const zkas = selectWalletNetwork(
...
test("disabling Experimental features revokes the active ZKas selection", () => {
  ...
  expect(
    isZKasActive({ ...zkas, networkId: NetworkType.TestnetT10 }, true),
  ).toBe(false);
});

test("a stale settings write cannot reauthorize ZKas after the toggle is turned off", () => {
  const enabled = selectWalletNetwork(
    { ...initialSettings, preview: true },
    ZKAS_MAINNET,
    true,
  );
  ...
  expect(getSelectedWalletNetwork(enabled, disabledFlag)).toBe(
    NetworkType.Mainnet,
  );
  expect(getVisibleWalletNetworks(enabled, disabledFlag)).not.toContain(
    ZKAS_MAINNET,
  );
  expect(() =>
